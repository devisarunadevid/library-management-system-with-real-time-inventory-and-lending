import { useState } from "react";
import {
  BookOpen,
  Crown,
  Sparkles,
  Star,
  Mail,
  ArrowRightCircle,
} from "lucide-react";
import { forgotPassword } from "../services/authService";
import { useNavigate } from "react-router-dom";

export default function ForgotPassword() {
  const [email, setEmail] = useState("");
  const navigate = useNavigate();

  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await forgotPassword(email);
      alert("OTP sent to your email!");
      navigate("/verify-otp");
    } catch (err) {
      alert("Failed to send OTP.");
      console.error(err);
    }
  };

  return (
    <div
      className="flex h-screen w-screen items-center justify-center relative"
      style={{
        backgroundImage: `url('/libraryshelf.jpeg')`,
        backgroundSize: "cover",
        backgroundPosition: "center",
        backgroundRepeat: "no-repeat",
        backgroundAttachment: "fixed",
      }}
    >
      <div className="absolute inset-0 bg-[rgba(20,14,5,0.45)] backdrop-blur-[2px]" />
      <div className="relative z-10 bg-white/85 backdrop-blur-lg p-8 rounded-3xl shadow-2xl w-full max-w-md border border-amber-200 ring-1 ring-white/30">
        <div className="flex justify-center mb-3">
          <div className="p-3 bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 text-white rounded-full shadow ring-1 ring-white/40">
            <BookOpen className="w-8 h-8" />
          </div>
        </div>

        <div className="mb-2 flex items-center justify-center gap-2 text-amber-500">
          <Crown className="w-5 h-5" />
          <Star className="w-5 h-5" />
          <Sparkles className="w-5 h-5" />
        </div>

        <h2 className="text-2xl font-extrabold text-center mb-1 bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 bg-clip-text text-transparent drop-shadow-[0_1px_1px_rgba(0,0,0,0.25)]">
          Forgot Password
        </h2>
        <p className="text-stone-700 text-center mb-6">
          Enter your registered email to receive a reset OTP
        </p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-amber-600">
              <Mail className="w-5 h-5" />
            </span>
            <input
              type="email"
              placeholder="Enter your email"
              value={email}
              onChange={(e) => setEmail(e.target.value)}
              className="w-full pl-10 pr-4 py-3 border border-amber-200/70 rounded-lg bg-white/90 focus:outline-none focus:ring-2 focus:ring-amber-400/60 placeholder:text-stone-400 shadow-sm"
              required
            />
          </div>

          <button
            type="submit"
            className="w-full inline-flex items-center justify-center gap-2 rounded-xl bg-gradient-to-r from-yellow-500 via-amber-500 to-orange-500 text-white py-3 font-semibold shadow hover:shadow-lg hover:scale-[1.01] active:scale-[0.99] transition-all ring-1 ring-white/30"
          >
            Send OTP
            <ArrowRightCircle className="w-5 h-5 text-white/95" />
          </button>
        </form>

        <p className="mt-6 text-sm text-center text-stone-700">
          Remember your password?{" "}
          <a
            href="/login"
            className="text-amber-600 font-semibold hover:underline"
          >
            Back to Login
          </a>
        </p>
      </div>
    </div>
  );
}
